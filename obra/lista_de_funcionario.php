...
<?php
    ...
    $sql = "SELECT * FROM funcao ORDER BY id ASC";
    ...
    <style>
      ...
      .fechar {
        ...
        /* right: 0; */
          ...
          /* Outros estilos para a div de fechamento, como cor e ícone de fechamento, etc. */
        }
      ...
      .textos {
        width: 100%; /* Defina a largura desejada, por exemplo, 80% da largura da janela */
        max-width: 800px; /* Defina a largura máxima, se necessário */
        height: 110%; /* Defina a altura desejada, por exemplo, 60% da altura da janela */
        max-height: 900px; /* Defina a altura máxima, se necessário */
        margin: 0 auto; /* Centralize o modal horizontalmente */
        overflow: auto; /* Adicione uma barra de rolagem, se o conteúdo for maior do que as dimensões do modal */
        background-color: rgb(18,19,23); /* Cor de fundo do modal */
        /* border: 1px solid #ccc; Adicione uma borda, se necessário */
        padding: 20px; 
      }
      #teste *{
  ...
  <body>
      ...
      <aside class="z-20 hidden w-64 overflow-y-auto bg-white dark:bg-gray-800 md:block flex-shrink-0">
        <div class="py-4 text-gray-500 dark:text-gray-400">
          ...
          <ul>
            ...
            <li class="relative px-6 py-3">
              <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200" href="lotes.php">
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- <li class="relative px-6 py-3">
              <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200" href="perfil.php">
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <li class="relative px-6 py-3">
              <a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200" href="contato.php">
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <main class="h-full overflow-y-auto">
          <div class="container px-6 mx-auto grid">
            <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
              Lista de Funcionários      <!--  antes chamando de Dashboard -->
            </h2>
            <!-- CTA -->
                ...
                <table class="w-full whitespace-no-wrap">
                  <thead>
                    <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                      ...
                      <th class="px-4 py-3">Tipo Pix</th>
                      <th class="px-4 py-3">Pix</th>
                    </tr>
                  </thead>
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800" id="teste">
                    <?php
                       while ($user_data = mysqli_fetch_assoc($result)) {
                        ...
                        $id_funcionario = $user_data['id_funcionario'];
                          ...
                          // Adicionandoa consulta para obter o ID da tabela funcionario
                          $sql_id_funcionario = "SELECT id FROM funcionario WHERE id = $id_funcionario";
                          $result_id_funcionario = $conexao->query($sql_id_funcionario);
                          
                          $funcionario_id = "ID não encontrado"; // Valor padrão se não houver correspondência.
                          
                          if ($result_id_funcionario && $result_id_funcionario->num_rows > 0) {
                              $id_data = $result_id_funcionario->fetch_assoc();
                              $funcionario_id = $id_data['id'];
                          }
                        ...
                        $id_pix = $user_data['id_funcionario'];
                        $sql_pix = "SELECT pix FROM funcionario WHERE id = $id_pix";
                        $result_pix = $conexao->query($sql_pix);
                    
                        $funcionario_pix = "Diária não encontrada"; // Valor padrão se não houver correspondência.
                    
                        if ($result_pix && $result_pix->num_rows > 0) {
                            $pix_data = $result_pix->fetch_assoc();
                            $funcionario_pix = $pix_data['pix'];
                        }

                        $id_tipo = $user_data['id_funcionario'];
                        $sql_tipo = "SELECT tipo_pix FROM funcionario WHERE id = $id_tipo";
                        $result_tipo = $conexao->query($sql_tipo);
                    
                        $funcionario_tipo = "Diária não encontrada"; // Valor padrão se não houver correspondência.
                    
                        if ($result_tipo && $result_tipo->num_rows > 0) {
                            $tipo_data = $result_tipo->fetch_assoc();
                            $funcionario_tipo = $tipo_data['tipo_pix'];
                        }
                              
                       ?>
                    <tr>
                    
                    <td class='espa text-white' style='text-align: left; padding: 10px; cursor: pointer;' onclick="acao('<?=$funcionario_id?>', '<?=$user_data['id']?>');"><?=$funcionario_nome?></td>
                   
                        <td class='espa text-white' style='text-align: left; padding: 15px'><?=$user_data['diaria']?></td>

                        <td class='espa text-white' style='text-align: left; padding: 15px'><?=$user_data['funcao']?></td>
                        <td class='espa text-white' style='text-align: left; padding: 15px'><?= $funcionario_tipo?></td>

                        <td class='espa text-white' style='text-align: left; padding: 10px'><?= $funcionario_pix?></td>
                        </tr>
                    <?php
                    } // Fim do loop while
                    ?>
    ...
    <script>
      function acao(id1,id2) {
        ...
        fetch(`edit.php?id1=${id1}&id2=${id2}`)
